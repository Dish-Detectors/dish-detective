...
describe("Create Account Server Actions", () => {
  ...
  describe("createEmployeeAccount", () => {
    ...
    it("should create a worker account successfully", async () => {
      const mockCreateUser = jest.fn().mockResolvedValue({ id: "clerk_new_user_id" });
    ...
    it("should create a manager account successfully", async () => {
      ...
      const mockCreateUser = jest.fn().mockResolvedValue({ id: "clerk_new_manager_id" });
      (clerkClient as unknown as jest.Mock).mockResolvedValue({
        users: {
          createUser: mockCreateUser,
        },
    ...
    it("should handle Clerk creation errors", async () => {
      const mockCreateUser = jest.fn().mockRejectedValue({ clerkError: true, errors: [] });
      (clerkClient as unknown as jest.Mock).mockResolvedValue({
        users: {
          createUser: mockCreateUser,
        },
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe("Greška prilikom kreiranja računa u Clerk sustavu");
    });
  });
});
