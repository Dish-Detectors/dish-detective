...
export async function getPoll(pollId: string) {
    try {
        ...
        const restaurant = await Restaurant.findById(poll.restaurantId).select("name").lean();
...
export async function submitPollAnswers(pollId: string, answers: { questionIndex: number; value: number }[]) {
    try {
        const { userId } = await auth();
        if (!userId) return { error: "Unauthorized" };
