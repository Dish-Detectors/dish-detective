...
export async function getEligibleStudentCount() {
  try {
    ...
    if (!restaurantId)
      return { count: 0, error: "Manager not assigned to any restaurant" };

    await dbConnect();
    const restaurant = await Restaurant.findById(restaurantId);
    if (!restaurant) return { count: 0, error: "Restaurant not found" };

    const availableDishIds = restaurant.availableDishes.map((id) =>
      id.toString(),
    );
...
export async function createPoll(formData: {
  ...
  try {
    ...
    if (!restaurantId)
      return { error: "Manager not assigned to any restaurant" };
    ...
    const usersToSelectCount = Math.max(
      1,
      Math.ceil(userIds.length * (formData.percentage / 100)),
    );
...
export async function getPollResults(pollId: string) {
  ...
  try {
    ...
    const answers = await (
      mongoose.models.Answers || mongoose.model("Answers")
    ).find({ pollId });
