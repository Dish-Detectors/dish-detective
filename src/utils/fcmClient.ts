...
import { subscribeTokenToTopic, unsubscribeTokenFromTopic } from "@/actions/notification";
...
export const unsubscribeFromDishTopic = async (menuItemId: string) => {
    ...
    try {
        const res = await unsubscribeTokenFromTopic(token, `dish_notify_${menuItemId}`);
        return res.success;
    } catch (error) {
